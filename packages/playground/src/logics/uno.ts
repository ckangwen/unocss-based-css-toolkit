...
export const transformedHTML = computedAsync(
  async () => await getTransformedHTML(),
)
...
export async function getHint(cm: Editor): Promise<Hints | undefined> {
  ...
  const result = await autocomplete.suggestInFile(
    cm.getDoc().getValue(),
    cursor,
  )
...
watch(transformedHTML, generate, { immediate: true })
