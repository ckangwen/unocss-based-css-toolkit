...
export function useResize(
  ...
  function onPointerMove(evt: PointerEvent) {
    ...
    isPathIncludesTarget.value = (evt.composedPath() as Element[]).includes(
      target.value,
    )
    ...
    widthRef.value = clamp(
      newWidth,
      Number(unref(minWidth)),
      Number(unref(maxWidth)),
    )
    heightRef.value = clamp(
      newHeight,
      Number(unref(minHeight)),
      Number(unref(maxHeight)),
    )

    transform.value
      = getComputedStyle(target.value).position === 'fixed'
        ? `translate(${
            direction.value.includes('left')
              ? clamp(
                  leftStart.value + xDiff,
                  leftStartMin.value,
                  leftStartMax.value,
                )
              : left
          }px,${
            direction.value.includes('top')
              ? clamp(
                  topStart.value + yDiff,
                  topStartMin.value,
                  topStartMax.value,
                )
              : top
  ...
  async function onPointerDown(evt: PointerEvent) {
    ...
    if (isOverEdge.value || (isOverEdge.value && evt.pointerType === 'touch')) {
      ...
      leftStartMax.value
        = clientRect.width - Number(unref(minWidth)) + clientRect.left
      topStartMax.value
        = clientRect.height - Number(unref(minHeight)) + clientRect.top
      leftStartMin.value
        = clientRect.left - (Number(unref(maxWidth)) - clientRect.width)
      topStartMin.value
        = clientRect.top - (Number(unref(maxHeight)) - clientRect.height)
  ...
  const isOnForeground = (
    ...
    const getCorner = (xA: number, yA: number) => {
      ...
      while (!found && Math.abs(xA - x) < unref(borderRadius) / 2) {
        ...
        yA += yReduce!
        found
          = target.value!.contains(window!.document.elementFromPoint(xA, yA))
          || window!.document.elementFromPoint(xA, yA) === target.value!
      }
      return found
    }
    const element = window!.document.elementFromPoint(x, y)
    return (
      isPathIncludesTarget.value
      || (element && target.value!.contains(element))
  ...
  function handlePointer({ currentX, currentY }: typeof pointer) {
    ...
    if (
      (currentY - top < 0
        ? Math.abs(currentY - top) < edgeWidthOutside
        : currentY - top <= edgeWidthInside)
      && (currentX - left < 0
        ? Math.abs(currentX - left) < edgeWidthOutside
        : currentX - left <= edgeWidthInside)
      ...
      (currentY - top < 0
        ...
        : currentY - top < edgeWidthInside)
      && (currentX - right > 0
        ? Math.abs(currentX - right) < edgeWidthOutside
        : Math.abs(currentX - right) < edgeWidthInside)
      ...
      (currentY - bottom > 0
        ? Math.abs(currentY - bottom) < edgeWidthOutside
        : Math.abs(currentY - bottom) < edgeWidthInside)
      && (currentX - left < 0
        ? Math.abs(currentX - left) < edgeWidthOutside
        : currentX - left < edgeWidthInside)
      ...
      ((currentX - left < 0 && Math.abs(currentX - left) < edgeWidthOutside)
        || (currentX - left >= 0
          && Math.abs(currentX - left) <= edgeWidthInside))
      && currentY > top
      && currentY < bottom
      && isEdgeActive('left')
